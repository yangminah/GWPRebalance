"""
This script contains all the helper functions needed for the second data structure I used. 
...
def get_batch(
    ...
    if load2mem is True:
        ...
        xloc[:bs, 2] = torch.from_numpy(
            data["sp"][tstart[idxst:idxend], la[idxst:idxend], lo[idxst:idxend]]
        )
...
def get_data(year, transform=None, load2mem=True, data={}):
    fid = Dataset(
        f"/scratch/epg2/mima-output/mima2.0-NH-optimal_d{year*360:05}/atmos_4xdaily.nc",
        "r",
    )
    if load2mem is True:
        if len(data) == 0:
            ...
            data["target"] = np.concatenate(
                (
                    np.array(fid["gwfu_cgwd"][()]).reshape(1440, 40, 64, 128, 1),
                    np.array(fid["gwfv_cgwd"][()]).reshape(1440, 40, 64, 128, 1),
                ),
                axis=-1,
            )
            ...
            data["sp"][:, :, :] = fid["ps"][()] / 100
            data["target"][:, :, :, :, 0] = np.array(fid["gwfu_cgwd"][()])
            data["target"][:, :, :, :, 1] = np.array(fid["gwfv_cgwd"][()])
            if transform:
                transform(data)
            return data
    else:
        data["input_3d"] = [fid["ucomp"], fid["vcomp"]]
        data["sp"] = fid["ps"]
        data["lat"] = np.array(fid["lat"][()])
        data["lon"] = np.array(fid["lon"][()])
        data["target"] = [fid["gwfu_cgwd"], fid["gwfv_cgwd"]]
        return data
