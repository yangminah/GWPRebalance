...
class model(nn.Module):
    def __init__(self, n_d=[128,64,32,1], n_in=40, n_out=33):
        ...
        self.shared = nn.Sequential(
            nn.Linear(self.n_in*2, self.n_d[0]),       nn.ReLU()
            )
        self.branches = nn.ModuleList([ nn.Sequential(
            nn.Linear(self.n_d[0], self.n_d[1]),       nn.ReLU(),
            nn.Linear(self.n_d[1], self.n_d[2]),       nn.ReLU(),
    ...
    def forward(self, x):
        ...
        ## ELSE
        # x3 = x
        # gu = torch.zeros(x3.shape[0],self.n_in)
